"""
This will read rtd and add temp compensation to the PH and DO sensors.
and will write the information to the console, and to a sql DB with a time and date
uses code from Atlas Scientific AtlasI2C.py under the MIT license
the rest is by John Roppel
Hardware used is the atlas PH, RTD and DO EZO boards and probes, the Whitebox raspberry pi carrier board.
...
def get_devices():
    ...
    for i in device_address_list:
        ...
        check_ezo = response.split(",")
        if len(check_ezo) > 0:
            if check_ezo[0].endswith("?I"):
                # yes - this is an EZO device
                moduletype = check_ezo[1]
